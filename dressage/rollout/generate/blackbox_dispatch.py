...
async def generate(
    ...
    try:
        ...
        register_session_context = getattr(
            proxy_client, "register_session_context", None
        )
        if callable(register_session_context):
            context_owned = True
            await maybe_await(register_session_context(session_id))
