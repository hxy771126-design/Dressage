...
GPU_UTIL_MONITOR="${REPO_ROOT}/examples/scripts/gpu_utilization_monitor.sh"
...
dressage_apply_common_defaults \
  "qwen3.5-35B-A3B-sync-local-l3-hicache" \
  "blackbox" \
  "local_bwrap"
...
# contributes one segment, so the 4GB default yields 16GB across 4 Engines.
...
WANDB_ARGS=(
  ...
  # --wandb-group qwen3.5-35B-A3B-dressage-sync-8gpu
  # --wandb-key "${WANDB_KEY}"
)
