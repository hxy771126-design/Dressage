...
print_plan() {
  ...
  echo "  engine count:  ${EXPECTED_ENGINE_COUNT}"
  echo "  SGLang static memory fraction: ${SGLANG_MEM_FRACTION_STATIC}"
...
values = {
    ...
    "expected_engine_count": expected_engine_count,
...
if "--sglang-enable-deterministic-inference" not in text:
    marker = "  --sglang-mem-fraction-static 0.6\n"
    if marker not in text:
