...
ENABLE_ENGINE_REBALANCING="${ENABLE_ENGINE_REBALANCING:-0}"
if [[ "${ENABLE_ENGINE_REBALANCING}" != "0" &&
      "${ENABLE_ENGINE_REBALANCING}" != "1" ]]; then
    echo "ENABLE_ENGINE_REBALANCING must be 0 or 1" >&2
    exit 1
fi

# Defaults preserve the original Blackbox workload. Benchmark wrappers can
# reuse the same 4B/Mooncake deployment with a proxy-only Whitebox generator.
...
export MC_MIN_RPC_PORT="${MC_MIN_RPC_PORT:-21000}"
export MC_MAX_RPC_PORT="${MC_MAX_RPC_PORT:-21999}"
...
DRESSAGE_ENGINE_REBALANCING_DEPLOYMENT_CONFIG="${DRESSAGE_ENGINE_REBALANCING_DEPLOYMENT_CONFIG:-${LOG_DIR}/proxy/${RUN_NAME}.rebalancing-deployment.json}"
if [[ "${ENABLE_ENGINE_REBALANCING}" == "1" ]]; then
    export DRESSAGE_ENGINE_REBALANCING_DEPLOYMENT_CONFIG
fi
...
if [[ -n "${MODEL_TOOL_CALL_TYPE}" ]]; then
  PROXY_ARGS+=(
...
ROLLOUT_ARGS=(
  ...
  --prompt-data \
    "${PROMPT_DATA:-${REPO_ROOT}/examples/data/dressage_dapo_prompts.jsonl}"
  --input-key prompt
  --label-key label
